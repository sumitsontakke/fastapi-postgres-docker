    ...
    try:
        ...
        if not find_ipmessage(db, **imsg_query):
            ...
            cleaned_imsg = {
                ...
                "amount": [0] if not imsg.get("amount", [0]) else imsg.get("amount", [0]),
...
def txnmsg_process(db):
    """Process imessages into application database"""
    this_imessage = iMessages(db=db)
    # Get all imessages from iMessages database (file)
    txn_messages = this_imessage.get_all_raw_messages()

    # filter messages having fi indicator in sender
    txn_messages = [msg for msg in txn_messages if "_fi" in msg[0]]
    log.info(f"Found Transaction messags \n{len(txn_messages)}")

    # process message to extract fields like amount, to_id, accounts_info
    processed_messages = [this_imessage.process_message(txn_message) 
                          for txn_message in txn_messages]
    log.info(f"Passing processed messages to sync with \n{len(processed_messages)}")
    
    # sync message into application database
    imsg_count = 0
    for imsg in processed_messages:
